...
ODE.rgbToHex = function rgbToHex(rgb) {
...
ODE.hexToRgb = function hexToRgb(hex) {
    hex = ODE.rgbToHex(hex);    // in case 'hex' is actually rgb!
...
// Calculate value, saturation and hue as in net.bhojpur.shoola.util.ui.colour.HSV
ODE.isDark = function(color) {
    if (color.endsWith('.lut')) {
        return false;
    }
    var c = ODE.hexToRgb(color);
...
ODE.openCenteredWindow = function(url, w, h) {
    var width = w ? +w : 550;
    var height = h ? +h : 600;
...
ODE.openScriptWindow = function(event, width, height) {
    ...
    if (script_url.indexOf('script_upload') > -1) {
        ODE.openCenteredWindow(script_url, width, 300);
        return;
    }

    // selected is list of {'id':'image-123'} etc.
    var selected = $("body").data("selected_objects.ode"),
        sel_types = {};
    ...
    ODE.openCenteredWindow(script_url, width, height);
    return false;
};
...
**/
ODE.get_tree_selection = function() {
 ...
 *    var OK_dialog = ODE.confirm_dialog("Can you confirm that you want to proceed?", function() {
 ...
 *    var title_dialog = ODE.confirm_dialog("Can you confirm that you want to proceed?",
 *          function() { alert( title_dialog.data("clicked_button") },
 *          "Dialog Title", btn_labels, 300, 200);
 */
ODE.confirm_dialog = function(dialog_text, callback, title, button_labels, width, height) {
...
ODE.alert_dialog = function(message) {
    ODE.confirm_dialog(message, undefined, "", ["OK"]);
};
 ...
 */
ODE.feedback_dialog = function(error, feedbackUrl) {
...
ODE.setupAjaxError = function(feedbackUrl){
    ...
    $(document).ajaxError(function(e, req, settings, exception) {
        ...
        if (req.status == 404) {
            ...
            ODE.feedback_dialog(error, feedbackUrl);
 ...
 */
ODE.login_dialog = function(login_url, callback) {
...
(function ($) {
    ...
    // Example usage:
    //
    //  $("#rotation_3d_tab").engine_right_plugin({           // The tab content element
    ...
    //  });
    $.fn.engine_right_plugin = function (settings) {
        ...
        this.each(function(i) {
            ...
            $("body").on("selection_change.ode", function(event) {
    ...
    $.fn.engine_center_plugin = function (settings) {
        ...
        this.each(function(i) {
            ...
            $('#center_panel_chooser').on('center_plugin_changed.ode', update_plugin_content);
            ...
            $("body").on("selection_change.ode", function(event) {
                ...
                var selected = $("body").data("selected_objects.ode");
                if (selected.length === 0) {
                    ODE.set_center_plugin_enabled(plugin_index, false);
                    return;
                }
                var obj_id = selected[0]['id'];     // E.g. image-123
                var orel = obj_id.split("-")[0];    // E.g. 'image'
                ...
                ODE.set_center_plugin_enabled(plugin_index, supported);
                if(supported) {
...
if (false) {                    // set to 'true' to run. NB: Need to uncomment 'console.log..' below.
    setTimeout(function(){      // use timeout to allow all scripts to load etc
        ...
        exceptions.push("sanitizeHexColor", "toRGB", "rgbToHex", "parseQuery", "downloadLandingDialog"); // from ode.gs_utils.js
