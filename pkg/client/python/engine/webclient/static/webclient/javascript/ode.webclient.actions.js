...
ODE.getURLParameter = function(key) {
...
ODE.linkify_element = function(elements) {
...
// called from ODE.tree_selection_changed() below
ODE.handle_tree_selection = function(data, event) {
    ...
    ODE.writeSelectedObjs(selected);

    // Trigger selection changed event
    $("body").trigger("selection_change.ode", data);

    // Instead of using selection_change.ode to trigger syncThumbSelection
...
ODE.tree_selection_changed = function(data, evt) {
    // handle case of deselection immediately followed by selection - Only fire on selection
    if (typeof ODE.select_timeout != 'undefined') {
        clearTimeout(ODE.select_timeout);
    }
    ODE.select_timeout = setTimeout(function() {
        ODE.handle_tree_selection(data, evt);
    }, 10);
};
...
ODE.clear_selected = function(force_refresh) {
    var refresh = (force_refresh === true);
    $("body")
        .data("selected_objects.ode", [])
        .trigger("selection_change.ode", [refresh]);
};

// select all images from the specified fileset (if currently visible)
ODE.select_fileset_images = function(filesetIds) {
...
ODE.share_selection_changed = function(share_id) {
    $("body").trigger("selection_change.ode");
};


// Standard ids are in the form TYPE-ID, web extensions may add an
// additional -SUFFIX
ODE.table_selection_changed = function($selected) {
    ...
    $("body")
        .data("selected_objects.ode", selected_objs)
        .trigger("selection_change.ode");
};

// handles selection for 'clicks' on table (search, history & basket)
// including multi-select for shift and meta keys
ODE.handleTableClickSelection = function(event) {
    ...
    if ( event.shiftKey ) {
        ...
        if ($s.length === 0) {
            ...
            ODE.table_selection_changed($clickedRow);
            return;
        }
    ...
    ODE.table_selection_changed($("table#dataTable tbody tr.ui-selected"));
};

// called from click events on plate. Selected wells
ODE.well_selection_changed = function($selected, well_index, plate_class) {
...
ODE.removeItem = function(event, domClass, url, parentId) {
    ...
    var confirm_remove = ODE.confirm_dialog(r + dType + '?',
        function() {
...
ODE.deleteItem = function(event, domClass, url) {
    ...
    var $parent = $(event.target).parents("."+domClass);
    var $annContainer = $parent.parent();
    var confirm_remove = ODE.confirm_dialog('Delete '+ dType + '?',
        function() {
            if(confirm_remove.data("clicked_button") == "OK") {
                $.ajax({
                    type: "POST",
                    ...
                    success: function(r){
                        if(eval(r.bad)) {
                            ...
                            // simply remove the item (parent class div)
                            $parent.remove();
                            window.parent.ODE.refreshActivities();
                        }
                    }
                });
            }
...
ODE.filterAnnotationsAddedBy = function() {
...
ODE.initToolbarDropdowns = function() {
    ...
    $("#figScriptList li a").on('click', function(event){
        if (!$(this).parent().hasClass("disabled")) {
            ODE.openScriptWindow(event, 800, 600);
        }
        event.preventDefault();
        return false;
    });
...
ODE.refreshThumbnails = function(options) {
    options = options || {};
    ...
    if (!options.ignorePreview && ODE.preview_viewport && ODE.preview_viewport.loadedImg.id) {
        ODE.preview_viewport.load(ODE.preview_viewport.loadedImg.id);
    }
};

ODE.load_thumbnails = function(thumbnails_url, input, batch, dthumb) {
    ...
    if (input.length > 0 && batch > 0) {
        ...
        if (iids.length > 0) {
            $.ajax({
                ...
                success: function(data){
                    ...
                    if (invalid_thumbs.length === iids.length && batch > 1) {
                        ODE.load_thumbnails(thumbnails_url, invalid_thumbs, 1, dthumb);
                    }
            ...
            ODE.load_thumbnails(thumbnails_url, input, batch, dthumb);
        }
    }
}
ODE.load_thumbnail = function(iid, thumbnails_url, callback) {
    // load thumbnails in a batches
    $.ajax({
        type: "GET",
        url: thumbnails_url,
...
ODE.checkForMultipleParents = function(objList, callback) {
...
ODE.handleDelete = function(deleteUrl, filesetCheckUrl, userId) {
    var datatree = $.jstree.reference($('#dataTree'));
    ...
    ODE.checkForMultipleParents(ajax_data, function(multiple){
        if (multiple) {
    ...
    $.get(filesetCheckUrl + "?" + ODE.get_tree_selection(), function(html){
        html = html.trim();
...
ODE.formatDate = function formatDate(date) {
...
ODE.nodeHasPermission = function(node, permission) {
    /*
    ...
    */
    ...
    if (Array.isArray(node)) {
        for (var index in node) {
            if (!ODE.nodeHasPermission(node[index], permission)) {
                return false;
            }
        }
...
ODE.writeSelectedObjs = function(selected_tree_nodes, selected_icons) {
/***
...
*/
    ...
    $("body").data("selected_objects.ode", selected_objs);
};

ODE.getTreeBestGuess = function(targetType, targetId) {
    /***
...
ODE.getTreeImageContainerBestGuess = function(imageId) {
...
ODE.formatScriptName = function(name) {
...
ODE.showScriptList = function(event) {
            ...
            var build_ul = function(script_data) {
                ...
                for (var i=0; i<script_data.length; i++) {
                    var li = script_data[i],   // dict of 'name' and 'ul' for menu items OR 'id' for scripts
                        name = li.name;
                    if (li.id) {
                        name = ODE.formatScriptName(name);
...
ODE.toggleFileAnnotationCheckboxes = function(event) {
...
ODE.fileAnnotationCheckboxChanged = function(event) {
    $(event.target).parents("li").toggleClass("selected");
};

ODE.fileAnnotationCheckboxDynamicallyAdded = function() {
...
ODE.copyRenderingSettings = function(rdef_url, selected) {
...
ODE.applyOwnerRenderingSettings = function(rdef_url, selected) {
    ODE.pasteRenderingSettings(rdef_url, selected);
};

ODE.resetRenderingSettings = function(rdef_url, selected) {
    ODE.applyRenderingSettings(rdef_url, selected);
};

// Paste settings from 'session' to selected Objects
ODE.pasteRenderingSettings = function(rdef_url, selected) {
    ODE.applyRenderingSettings(rdef_url, selected);
};

ODE.applyRenderingSettings = function(rdef_url, selected) {
    ...
    var rdef_confirm_dialog = ODE.confirm_dialog(
        confirmMsg,
        function() {
...
ODE.setPaneExpanded = function setPaneExpanded(name, expanded) {
...
ODE.getPaneExpanded = function getPaneExpanded(name) {
