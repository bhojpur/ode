(function(){
    ...
    window.channelChange = function (ev, obj, idx, ch) {
        if (ch.active) {
            ...
            $('#wbode-ch'+idx).addClass('pressed');
            $('#rd-wbode-ch'+idx).addClass('pressed');
        } else {
            $('#wbode-ch'+idx).removeClass('pressed');
            $('#rd-wbode-ch'+idx).removeClass('pressed');
        }
    };


    window.imageChange = function (viewport) {
        $('#wbode-t-curr').html(viewport.getTPos());
        $('#wbode-z-curr').html(viewport.getZPos());
        $('#wbode-t-count').html(viewport.getTCount());
        $('#wbode-z-count').html(viewport.getZCount());

        if (viewport.hasLinePlot() || $('#wbode-lp-enable').prop('checked')) {
    ...
    function getLutIndex(lutName) {
      if (ODE && ODE.LUTS) {
        for (var l=0; l<ODE.LUTS.length; l++) {
          if (ODE.LUTS[l].name === lutName) {
            return ODE.LUTS[l].png_index;
          }
        }
      }
      return -1;
    ...
    function getLutBgPos(color, slider) {
        var png_height = ODE.PNG_LUTS.length * 10;
        ...
        if (color.endsWith('.lut')) {
            ...
            if (slider) {
                yoffset = '-' + ((ODE.LUTS.length) * 30 + 7) + 'px';
            } else {
    ...
    window.syncRDCW = function(viewport) {
        ...
        for (i=0; i<channels.length; i++) {
            ...
            $('#wbode-ch' + i).css('background-color', '#' + color)
                .find('.lutBackground').css(lutBgStyle);
            // Slider background
            $('#wbode-ch'+i+'-cwslider').find('.ui-slider-range').addClass('lutBackground')
            ...
            $('#wbode-ch'+i+'-color').attr('data-color', channels[i].color);
            $('#wbode-ch' + i + '-cw-start').val(channels[i].window.start).trigger('change');
            $('#wbode-ch' + i + '-cw-end').val(channels[i].window.end).trigger('change');
        }
        ...
        updateUndoRedo(viewport);
        $('#rd-wbode-rmodel').prop('checked', viewport.isGreyModel());
    ...
    window.applyRDCW = function(viewport, final) {
        ...
        for (var i=0; i<viewport.getCCount(); i++) {
            active = $('#rd-wbode-ch'+i).get(0).checked;
            if (active !== viewport.loadedImg.channels[i].active) {
                viewport.setChannelActive(i, active, true);
            }
            viewport.setChannelColor(i, $('#wbode-ch'+i+'-color').attr('data-color'), true);
            revInt = $('#wbode-ch'+i+'-color').data('data-reverse-intensity');
            ...
            viewport.setChannelWindow(i, $('#wbode-ch'+i+'-cw-start').get(0).value, $('#wbode-ch'+i+'-cw-end').get(0).value, noreload);
        }
    ...
    window._load_metadata = function(ev, viewport) {
        ...
        $('#wbode-image-name').html(tmp.imageName.escapeHTML());
        $('#wbode-image-description-content').html(tmp.imageDescription.escapeHTML().replace(/\n/g, '<br />'));
        $('#wbode-image-author').html(tmp.imageAuthor.escapeHTML());
        $('#wbode-image-pub').html(tmp.projectName.escapeHTML());
        $('#wbode-image-pubid').html(tmp.projectId);
        $('#wbode-image-timestamp').html(tmp.imageTimestamp);
    ...
    */
    window._refresh_cb = function (ev, viewport) {
        ...
        $('#wbode-rmodel').prop('checked', viewport.isGreyModel());
        $('#wbode-invaxis').prop('checked', viewport.loadedImg.rdefs.invertAxis);

        var q = viewport.getQuality();
        if (q) {
            var qr = $('#wbode-quality > option[value="' + q.toFixed(1) + '"]');
            if (qr.length) {
        ...
        if (channels.length < 2) {
            $("#wbode input[value='split']").prop('disabled', true);
        }

        // TODO: this used anywhere?
        // {% block xtra_metadata %}{% endblock %}

        /*$('#wbode-shortname').attr('title', tmp.imageName).html(gs_text_trim(tmp.imageName, 15, true));*/

        tmp = viewport.getSizes();
        $('#wbode-image-width').html(tmp.width);
        $('#wbode-image-height').html(tmp.height);
        $('#wbode-image-z-count').html(tmp.z);
        $('#wbode-image-t-count').html(tmp.t);
        tmp = viewport.getPixelSizes();
        $('#wbode-image-pixel-size-x').html(tmp.x===null?'-':(tmp.x.lengthformat()));
        $('#wbode-image-pixel-size-y').html(tmp.y===null?'-':(tmp.y.lengthformat()));
        $('#wbode-image-pixel-size-z').html(tmp.z===null?'-':(tmp.z.lengthformat()));

        if (tmp.x!==0) {
            $("#wbode-scalebar").prop("disabled", false);
        }
        ...
        var template = '' +
          ...
          '<td><button id="rd-wbode-ch$idx0" class="rd-wbode-ch squared $class" style="background-color: $col" ' +
            'title="$label"><div class="lutBackground"></div><div class="btnLabel">$l</div></button></td>' +
          '<td><table><tr id="wbode-ch$idx0-cw" class="rangewidget"></tr></table></td>' +
          '<td><button id="wbode-ch$idx0-color" class="picker squarred" title="Choose Color">&nbsp;</button></td>' +
          '</tr>';

        $('#rdef-postit table').on('focus', '.rangewidget input', function(){
            // id is wbode-ch1-cw-start or wbode-ch1-cw-end
            var chIdx = this.id.replace('wbode-ch', '').split('-')[0];
        ...
        var slide_cb = function() {
            return function(event, ui) {
                ...
                if (ui.values[0] !== s) {
                    $('#wbode-ch' + $(event.target).data('channel-idx') + '-cw-start').val(ui.values[0]).trigger('change');
                } else if (ui.values[1] !== e) {
                    $('#wbode-ch'+ $(event.target).data('channel-idx') + '-cw-end').val(ui.values[1]).trigger('change');
                }
        ...
        for (i=channels.length-1; i>=0; i--) {
            ...
            if (ODE.isDark(channels[i].color)) {
                btnClass += " fontWhite";
            }
            ...
            $('#wbode-ch'+(i)+'-cw').append('<td width="10%"><span class="min" title="min: ' + channels[i].window.min + '"><input type="text" id="wbode-ch' + i + '-cw-start" /></span></td><td><div class="rangeslider" id="wbode-ch' + i + '-cwslider"></div></td> <td width="10%"><span class="max" title="max: ' + channels[i].window.max + '"><input type="text" id="wbode-ch' + i + '-cw-end" /></span></td>');
            init_ch_slider(i, channels);
            $('#wbode-ch'+i+'-cw-start').val(channels[i].window.start).off('change').on('change', start_cb(i));
            $('#wbode-ch'+i+'-cw-start').on('keyup', keyup_cb()).on('focusout', focusout_cb());
            $('#wbode-ch'+i+'-cw-end').val(channels[i].window.end).off('change').on('change', end_cb(i));
            $('#wbode-ch'+i+'-cw-end').on('keyup', keyup_cb()).on('focusout', focusout_cb());
        }

        // bind clicking on channel checkboxes
        $(".rd-wbode-ch").each(function(i){
        ...
        $('#wbode-workarea > .box > div.row').show();
    };
