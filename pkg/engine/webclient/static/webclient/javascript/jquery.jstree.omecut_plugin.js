...
(function ($, undefined) {
    ...
    // Variable to store the ode specific cut status
    var odecut = false;
    ...
    $.jstree.plugins.odecut = function (options, parent) {
        ...
        this.cut = function (obj) {
            ...
            $.each(tmp, function(index, node) {
                ...
                odecut_buffer.push(new_node_data);
            });

            // Set the buffers including the ode flag
            ccp_mode = 'move_node';
            ccp_inst = inst;
            ccp_node = odecut_buffer;
            odecut = true;
                ...
                if (parent.type !== 'experimenter' &&
                    ...
                    $.when(unlinkNode(inst, node, parent)).done(function(rsp) {
                        ...
                        if (orphaned && !wrongTree) {
                            ...
                            ODE.updateNodeChildCount(inst, parent);
                        }
        ...
        this.can_paste = function () {
            var inst = this;
            return (odecut && ccp_node.length > 0) || parent.can_paste.call(this);
        };

        this.paste = function (obj, pos) {
            var inst = this;

            // If we've just done an 'odecut' (above), then custom paste ONLY creates new links
            if (odecut && ccp_node.length > 0) {
        ...
        // we have to manually check in the case of this being an odecut
        this.check = function (chk, obj, par, pos, more) {
            // If this is an odecut and the object does not have an instance (i.e. it
    ...
    // $.jstree.defaults.plugins.push("odecut");
})(jQuery);