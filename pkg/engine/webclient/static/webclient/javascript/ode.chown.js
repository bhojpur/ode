$(function() {
    ...
    window.ODE.handleChown = function() {
        // gid, gname, oid
        ...
        selobjs = ODE.get_tree_selection().split("&");  // E.g. Image=1,2&Dataset=3
            ...
            var getDryRun = function () {
                ...
                $.get(url, data, function (dryRunData) {
                    if (dryRunData.finished) {
                        ...
                        // formatDryRun is in ode.chgrp.js
                        var showParents = true;
                        let html = ODE.formatDryRun(dryRunData, showParents);
    ...
    $chownform.ajaxForm({
        beforeSubmit: function(data, $form){
            ...
            if (owner_data.length === 0) {
                ODE.alert_dialog("Please choose target user.");
                return false;
            }
        },
