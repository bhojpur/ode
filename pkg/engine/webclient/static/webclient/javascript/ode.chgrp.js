...
$(function() {
    ...
    if (typeof window.ODE === "undefined") { window.ODE={}; }
    ...
    window.ODE.handleChgrp = function(webindex, staticurl) {
        ...
        var url = webindex_url + "load_chgrp_groups/?" + ODE.get_tree_selection();
    ...
    var checkFilesetSplit = function checkFilesetSplit () {
        ...
        var sel = ODE.get_tree_selection(),
        ...
        $.get(webindex_url + "fileset_check/chgrp/?" + sel, function(html){
            ...
            if($('div.split_fileset', html).length > 0) {
                ...
                if (selImages) {
                    ODE.select_fileset_images(filesetIds);
                }
            ...
            var getDryRun = function() {
                ...
                $.get(url, data, function(dryRunData) {
                    if (dryRunData.finished) {
                        ...
                        let html = ODE.formatDryRun(dryRunData);
    ...
    $chgrpform.on( "click", ".chgrpGroup", function() {
        ...
        var selobjs = ODE.get_tree_selection().split("&");  // E.g. Image=1,2&Dataset=3
    ...
    $chgrpform.ajaxForm({
        beforeSubmit: function(data, $form){
            ...
            if (data.length === 0) {
                ODE.alert_dialog("Please choose target group.");
                return false;
            }
    ...
    window.ODE.formatDryRun = function (dryRunData, showParents) {
        var html = "",
            move = [], count,
            unlink = [];
