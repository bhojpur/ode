...
$(function() {
    ...
    .on('changed.jstree', function (e, data) {
        ...
        ODE.tree_selection_changed(data, e);
        if (ODE.hideWellBirdsEye) {
            ODE.hideWellBirdsEye();
        }
    })
    .on('selection_change.ode', function(e, nElements) {
    ...
    .on('copy_node.jstree', function(e, data) {
        ...
        */
        ...
        ODE.updateNodeChildCount(inst, data.parent);
    ...
    .on('move_node.jstree', function(e, data) {
        ...
        */
        ...
        // trigger ode remove
        $("body").trigger('removed_node.jstree.ode', data.node.data.obj);
        ...
        ODE.updateNodeChildCount(inst, data.parent);
        ODE.updateNodeChildCount(inst, data.old_parent);
    ...
    .jstree({
        'plugins': ['types', 'contextmenu', 'dnd', 'sort', 'locate',
                    'odetools', 'conditionalselect', 'pagination', 'fields',
                    'truncatetext', 'childcount', 'odecut'],
        // The jstree core
        'locate' : {
        ...
        'core' : {
            ...
            'check_callback': function(operation, node, node_parent, node_position, more) {
                ...
                if (more && more.core &&
                    ...
                    if (!ODE.nodeHasPermission(node_parent, 'canLink') &&
            ...
            'is_draggable': function(nodes) {
                ...
                for (var index in nodes) {
                    ...
                    if (!inst.get_rules(node).draggable ||
                          !ODE.nodeHasPermission(node, 'canLink') ||
            ...
            'items' : function(node){
                ...
                if (tagTree) {
                    config["create"]["submenu"] = {
                        "tagset": {
                            ...
                            "_disabled" : true,
                            "icon"      : WEBCLIENT.URLS.static_webclient + 'image/left_sidebar_icon_tags.png',
                            action      : function (node) {ODE.handleNewContainer("tagset"); },
                        },
                        "tag": {
                            "label"     : "Tag",
                            "_disabled" : true,
                            "icon"      : WEBCLIENT.URLS.static_webclient + 'image/left_sidebar_icon_tag.png',
                            action      : function (node) {ODE.handleNewContainer("tag"); },
                        }
                    };
                } else {
                    config["create"]["submenu"] = {
                        "project": {
                            ...
                            "_disabled": true,
                            "icon"  : WEBCLIENT.URLS.static_webclient + 'image/folder16.png',
                            action: function (node) {ODE.handleNewContainer("project"); },
                        },
                        "dataset": {
                            "label" : "Dataset",
                            "_disabled": true,
                            "icon"  : WEBCLIENT.URLS.static_webclient + 'image/folder_image16.png',
                            action: function (node) {ODE.handleNewContainer("dataset"); },
                          },
                          "screen": {
                            "label" : "Screen",
                            "_disabled": true,
                            "icon"  : WEBCLIENT.URLS.static_webclient + 'image/folder_screen16.png',
                            action: function (node) {ODE.handleNewContainer("screen"); },
                          }
                    };
                }
                ...
                config["delete"] = {
                    ...
                    "action": function(){
                        ...
                        ODE.handleDelete(deleteUrl, filesetCheckUrl, WEBCLIENT.USER.id);
                    }
                };
                ...
                config["chgrp"] = {
                    ...
                    "action": function() {
                        ...
                        ODE.handleChgrp(WEBCLIENT.URLS.webindex, WEBCLIENT.URLS.static_webclient);
                    }
                };
                ...
                config["chown"] = {
                    ...
                    "icon"  : WEBCLIENT.URLS.static_webclient + 'image/icon_basic_user_16.png',
                    "action": function() {
                        ODE.handleChown(WEBCLIENT.URLS.webindex, WEBCLIENT.URLS.static_webclient);
                    }
                };
                ...
                config["share"] = {
                    ...
                    "_disabled": function(){
                        ...
                        $.each(selected, function(index, node) {
                            if (node.type != 'image' || !ODE.nodeHasPermission(node, 'canLink')) {
                                enabled = false;
                                // Break out of $.each
                                return false;
                            }
                ...
                config["renderingsettings"] = {
                    ...
                    "submenu" : {
                        "copy_rdef"  : {
                            ...
                            "action": function() {
                                var inst = $.jstree.reference('#dataTree');
                                ODE.copyRenderingSettings(WEBCLIENT.URLS.copy_image_rdef_json,
                        ...
                        "paste_rdef": {
                            ...
                            "action": function() {
                                var inst = $.jstree.reference('#dataTree');
                                ODE.pasteRenderingSettings(WEBCLIENT.URLS.copy_image_rdef_json,
                                    inst.get_selected(true));
                            }
                        },
                        "reset_rdef": {
                            ...
                            "action": function() {
                                var inst = $.jstree.reference('#dataTree');
                                ODE.resetRenderingSettings(WEBCLIENT.URLS.reset_rdef_json,
                                    inst.get_selected(true));
                            }
                        },
                        "owner_rdef": {
                            ...
                            "action": function() {
                                var inst = $.jstree.reference('#dataTree');
                                ODE.applyOwnerRenderingSettings(WEBCLIENT.URLS.reset_owners_rdef_json,
                                    inst.get_selected(true));
                            }
                        }
                    }
                ...
                if (WEBCLIENT.OPEN_WITH.length > 0) {
                    ...
                    var viewers = WEBCLIENT.OPEN_WITH.map(function(v){
                        ...
                        return {
                            ...
                            "_disabled": function() {
                                if (!ODE.openWithDisabledByAjax) {
                                    ODE.openWithDisabledByAjax = {};
                                }
                                ...
                                if (typeof v.isEnabled === "function") {
                                    ...
                                    if (ODE.openWithDisabledByAjax[v.id] && ODE.openWithDisabledByAjax[v.id][selKey] !== undefined) {
                                        return ODE.openWithDisabledByAjax[v.id][selKey];
                                    }
                                    ...
                                    enabled = v.isEnabled(selJson, function(enable) {
                                        ...
                                        ODE.openWithDisabledByAjax[v.id] = {}
                                        ODE.openWithDisabledByAjax[v.id][selKey] = !enable;
                                    ...
                                    // ODE.openWithDisabledByAjax[v.id] = !enabled;
                                    return !enabled;
                                }
                ...
                var userId = WEBCLIENT.active_user.id,
                    ...
                    canLink = ODE.nodeHasPermission(node, 'canLink'),
                ...
                if (ODE.nodeHasPermission(node, 'canDelete')) {
                    config["delete"]["_disabled"] = false;
                }

                // Enable 'Move to group' if 'canChgrp'
                if(ODE.nodeHasPermission(node, 'canChgrp')) {
                ...
                if (ODE.nodeHasPermission(node, 'canAnnotate')) {
