...
$(function(){
    ...
    ODE.emptyWellBirdsEye = function() {
        $("#well_birds_eye").empty()
            .off("click");
    };
    ODE.hideWellBirdsEye = function() {
        $("#tree_details").css('height', '100%');
        $("#well_details").css('height', '0').css('display', 'none');

        // Also clear content
        ODE.emptyWellBirdsEye();
    };
    $("#hide_well_birds_eye").on('click', ODE.hideWellBirdsEye);

    ODE.WellBirdsEye = function(opts) {
    ...
    window.changeField = function changeField(field) {
        ...
        var containerNode = datatree.find_odepath(containerPath);

        if (!containerNode) {
            console.log('WARNING: Had to guess container');
            containerNode = ODE.getTreeBestGuess(containerType, containerId);
        }
    ...
    var primaryIndex = -1;
    ODE.handleClickSelection = function (event, target, elementsSelector) {
        ...
        else if (event && event[ODE.multi_key() + "Key"]) {
            if ( primaryIndex === -1 ) {
                primaryIndex = selIndex;
            }
