...
$.fn.roi_display = function(options) {
    return this.each(function(){
        ...
        var canvas_class = (options.canvas_class ? options.canvas_class : 'webode-viewport-roi');
        ...
        get_active_rois = function () {
            ...
            // merge ROIs coming from ODE server and external ROIs
            var global_rois = [];
            $.merge(global_rois, roi_json);
            if (external_rois)
                $.merge(global_rois, external_rois);
        ...
        // get json of all ROI, ODE ones and external ones, as a unique list
        ...
        ROI ID is valid if it is not alredy in use by an ODE ROI.
        ...
        var check_ext_shape_id = function(roi_id, shape_id) {
            // check if ROI ID is already used by one on ODE's ROIs...
            for (var rx=0; rx<roi_json.length; rx++) {
                if (roi_json[rx]["id"] == roi_id) {
                    console.error("ID " + roi_id + " already used by one of ODE ROIs");
                    return false;
                }
            }
        ...
        // configure a shape obtained using function in ode.roiutils.js as a proper shape for ODE.web viewer
        var configure_shape = function(shape_id, shape_config) {
            shape_config["id"] = shape_id;
        }

        // build a proper ROI definition that can be used by ODE.web viewer
        ...
        /*
        Add a SHAPE definition to ODE.web viewer. Shape will be associated to ROI with ID roi_id
        ...
        If hide_ode_rois is TRUE, hide ODE ROI when refreshing viewport.
         */
        this.add_external_shape = function(roi_id, shape_id, shape_config, refresh_rois, hide_ode_rois) {
            if (roi_json == null) {
                load_rois(false, undefined, function() {
                    $viewportimg.trigger("add_external_shape",
                        [roi_id, shape_id, shape_config, refresh_rois, hide_ode_rois]);
                });
            } else {
        ...
        $viewportimg.on("add_external_shape", function (event, roi_id, shape_id, shape_config,
            ...
            if (refresh) {
                // if hide_ode_rois is true, deactivate all ROIs coming from ODE before refresh
                var hide_oroi = typeof hide_ode_rois !== "undefined" ? hide_ode_rois : false;
        ...
        */
        ...
        this.hide_rois = function(hide_ode_rois, hide_external_rois) {
            var hide_ode_rois = typeof hide_ode_rois !== "undefined" ? hide_ode_rois : true;
            var hide_external_rois = typeof hide_external_rois !== "undefined" ? hide_external_rois : true;

            if (!hide_ode_rois && !hide_external_rois) {
                console.warn("HIDE_ODE_ROIS and HIDE_EXTERNAL_ROIS are False, nothing to do");
                return;
            }
        ...
        this.update_shape_text = function (roi_id, shape_id, text_value, font_family, font_size, font_style,
            ...
            // look for shape in ODE ROIs and external ones
