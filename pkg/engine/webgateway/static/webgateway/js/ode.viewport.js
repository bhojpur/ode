/**
 * webode-thumbslider - Webode image viewport
 */
...
if (!ODE) {
  ...
  jQuery.fn.WebodeViewport = function (server, options) {
    return this.each
    (
     function () {
       jQuery._WebodeViewport (this, server, options);
     });
  };
  
  jQuery.WebodeViewport = function (elm, server, options) {
    var container = jQuery(elm).get(0);
    var rv = container.WebodeViewport || (container.WebodeViewport = new jQuery._WebodeViewport(container, server, options));
    return rv;
  };
      ...
      if (this.rdefs.model === other.rdefs.model) {
        for (var i in this.channels) {
          if (this.channels[i].active != other.channels[i].active ||
              ODE.rgbToHex(this.channels[i].color) != ODE.rgbToHex(other.channels[i].color) ||
  ...
  /**
   * _WebodeViewport class created in the jQuery namespace holds all logic for interfacing with the webode
  ...
  jQuery._WebodeViewport = function (container, server, options) {
    if (server.endsWith("/")) {
    ...
    var ajaxTimeout;
    var tileHref;
    this.self.append('<div id="'+topid+'" id="'+topid+'" class="webode-viewport-top">');
    this.top = jQuery('#'+topid);
    this.top.append('<div id="'+zsliderid+'">');
    this.zslider = jQuery('#'+zsliderid);
    this.top.append('<div id="'+viewportid+'" class="webode-viewport-vp">');
    this.viewport = jQuery('#'+viewportid);
    this.viewport.append('<img id="'+viewportimgid+'">');
    this.viewportimg = jQuery('#'+viewportimgid);
    this.viewport.append('<div id="'+viewportmsgid+'" class="webode-viewport-msg"></div>');
    this.viewportmsg = jQuery('#'+viewportmsgid);
    this.self.append('<div id="'+bottomid+'" class="webode-viewport-bot">');
      ...
      if (_this.loadedImg.tiles) {
          ...
          var cx = _this.loadedImg.current.query.x,
            ...
            if (typeof init_zoom === "undefined") {
              ...
              while (init_zoom > 0) {
                var ode_zm_index = (zoom_levels-1)-init_zoom,   // convert PanoJs to ODE
                  scale = zoomLevelScaling[ode_zm_index],
  ...
  //      }
    ...
    this.getQuery = function (include_slider_pos, include_xy_pos, include_zoom) {
      ...
      for (var i=0; i<channels.length; i++) {
        ...
        ch += '$' + ODE.rgbToHex(channels[i].color);
        chs.push(ch);
