...
<!-- These paths are not correctly qualified but they are not correct in the LODE schema -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	exclude-result-prefixes="LODE AML CLI MLI STD Bin CA"
	...
	-->
	<xsl:template match="LODE:Screen">
	...
	-->
	<xsl:template match="LODE:Experiment">
		<xsl:element name="Experiment" namespace="{$newODENS}">
			<xsl:for-each select="@*">
				<xsl:attribute name="{local-name(.)}">
					<xsl:choose>
	...
	-->
	<xsl:template match="LODE:ChannelInfo">
		<xsl:element name="LogicalChannel" namespace="{$newODENS}">
			<xsl:for-each select="@*">
				<xsl:choose>
	...
	-->
	<xsl:template match="LODE:LightSource">
		<xsl:element name="LightSource" namespace="{$newODENS}">
	...
	-->
	<xsl:template match="LODE:Laser">
		<xsl:element name="Laser" namespace="{$newODENS}">
	...
	-->
	<xsl:template match="LODE:Arc">
		<xsl:element name="Arc" namespace="{$newODENS}">
			<xsl:for-each select="@* [not(name() = 'Power')]">
				<xsl:choose>
					<xsl:when test="local-name(.)='Type'">
						<xsl:attribute name="{local-name(.)}">
	...
	-->
	<xsl:template match="LODE:Filament">
		<xsl:element name="Filament" namespace="{$newODENS}">
			<xsl:for-each select="@* [not(name() = 'Power')]">
				<xsl:choose>
					<xsl:when test="local-name(.)='Type'">
						<xsl:attribute name="{local-name(.)}">
	...
	-->
	<xsl:template match="LODE:Detector">
		<xsl:element name="Detector" namespace="{$newODENS}">
			<xsl:for-each select="@*">
				<xsl:choose>
					<xsl:when test="local-name(.)='Type'">
						<xsl:attribute name="{local-name(.)}">
	...
	-->
	<xsl:template match="LODE:Objective">
		<xsl:element name="Objective" namespace="{$newODENS}">
			<xsl:apply-templates select="@*"/>
			<xsl:element name="Correction" namespace="{$newODENS}">Unknown</xsl:element>
			<xsl:element name="Immersion" namespace="{$newODENS}">Unknown</xsl:element>
			<xsl:for-each select="*  [local-name(.) = 'LensNA']">
				<xsl:element name="LensNA" namespace="{$newODENS}">
					<xsl:value-of select="."/>
				</xsl:element>
			</xsl:for-each>
			<xsl:for-each select="*  [local-name(.) = 'Magnification']">
				<xsl:element name="CalibratedMagnification" namespace="{$newODENS}">
					<xsl:value-of select="."/>
				</xsl:element>
			</xsl:for-each>
		</xsl:element>
	  ...
	  <xsl:variable name="filterID" select="@ID"/>
		...
		<xsl:for-each select="*  [local-name(.) = 'ExFilter']">
			...
			<xsl:element name="FilterSet" namespace="{$newODENS}">
				...
				<xsl:attribute name="Manufacturer">GeneratedByODEXslt</xsl:attribute>
				<xsl:attribute name="Model">2018-FC-to-2008-9</xsl:attribute>
			</xsl:element>
		</xsl:for-each>
	</xsl:template>
	...
	-->
	<xsl:template match="LODE:Filter" mode="OnlyFilter">
	  <xsl:variable name="filterID" select="@ID"/>
		<xsl:for-each select="*  [local-name(.) = 'ExFilter']">
			<xsl:element name="Filter" namespace="{$newODENS}">
	...
	-->
	<xsl:template match="LODE:Filter" mode="OnlyDichroic">
	  <xsl:variable name="filterID" select="@ID"/>
		<xsl:for-each select="*  [local-name(.) = 'Dichroic']">
			<xsl:element name="Dichroic" namespace="{$newODENS}">
	...
	-->
	<xsl:template match="LODE:FilterRef">
		<xsl:variable name="filterRefID" select="@ID"/>
		<xsl:element name="FilterSetRef" namespace="{$newODENS}">
	...
	-->
	<xsl:template match="LODE:OTF">
		<xsl:element name="OTF" namespace="{$newODENS}">
	...
	-->
	<xsl:template match="LODE:ScreenRef">
	...
	-->
	<xsl:template match="LODE:PlateRef">
	...
	-->
	<xsl:template match="LODE:Projection">
		<xsl:element name="Projection" namespace="{$newODENS}">
			<xsl:apply-templates select="node()"/>
				<xsl:for-each select="@*">
				<xsl:choose>
	...
	-->
	<xsl:template match="LODE:Time">
		<xsl:element name="Time" namespace="{$newODENS}">
			<xsl:apply-templates select="node()"/>
				<xsl:for-each select="@*">
				<xsl:choose>
	...
	Instrument
	-->
	<xsl:template match="LODE:Instrument">
		<xsl:element name="Instrument" namespace="{$newODENS}">
			<xsl:apply-templates select="@*"/>
	...
	Convert Pixels
	-->
	<xsl:template match="LODE:Pixels">
		<xsl:element name="Pixels" namespace="{$newODENS}">
			...
			<xsl:for-each select="ancestor::LODE:Image">
	...
	Convert TiffData
	-->
	<xsl:template match="LODE:TiffData" mode="OnlyTiffData">
		<xsl:element name="TiffData" namespace="{$newODENS}">
			<xsl:apply-templates select="@*|node()"/>
		</xsl:element>
	</xsl:template>
	...
	Skip TiffData
	-->
	<xsl:template match="LODE:TiffData" mode="OnlyBinData">
	...
	Convert Feature
	-->
	<xsl:template match="LODE:Feature">
		<xsl:element name="Region" namespace="{$newODENS}">
	...
	Convert ROI
	-->
	<xsl:template match="LODE:ROI">
		<xsl:variable name="parentID">
			<xsl:for-each select="ancestor::LODE:DisplayOptions">
				<xsl:for-each select="@* [name() = 'ID']">
					<xsl:value-of select="."/>
				</xsl:for-each>
			</xsl:for-each>
		...
		<xsl:element name="ROI" namespace="{$newODENS}">
			...
			<xsl:element name="Union" namespace="{$newODENS}">
				<xsl:choose>
					<xsl:when test="(($theMaxT = $theMinT) and (($theMaxZ = $theMinZ)))">
						<xsl:element name="Shape" namespace="{$newODENS}">
							...
							<xsl:element name="Rect" namespace="{$newODENS}">
	...
	Rename ChannelInfo, remove element AuxLightSourceRef, process enums
	-->
	<xsl:template match="LODE:Image">
		<xsl:element name="Image" namespace="{$newODENS}">
			<xsl:for-each select="@*">
				<xsl:choose>
	...
	<!--
	Move all AML:Description Elements the Bhojpur ODE namespace
	-->
	<xsl:template match="AML:Description">
		<xsl:element name="Description" namespace="{$newODENS}">
			<xsl:apply-templates select="node()"/>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template match="LODE:*">
		<xsl:element name="{name()}" namespace="{$newODENS}">
			<xsl:apply-templates select="@*|node()"/>
		</xsl:element>
	</xsl:template>
