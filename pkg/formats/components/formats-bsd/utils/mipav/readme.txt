MIPAV plugin for opening life sciences images using ODE-Formats.

-------------------------------------------------------------------------------
Steps to try out the plugin:

1) Download the odeformats_package.jar trunk build from:
     https://www.bhojpur.net/ode-formats/downloads
   and save it in your MIPAV directory.

2) Compile the plugin with:
     javac -cp /path/to/mipav:/path/to/mipav/odeformats_package.jar \
       PlugInOdeFormatsImporter.java
   where "/path/to/mipav" is the location of your MIPAV installation.

3) Copy the resultant PlugInOdeFormatsImporter*.class files into your user
...
4) Add odeformats_package.jar to MIPAV's class path:
     * On Mac OS X, edit the mipav.app/Contents/Info.plist file.
     * On Windows, edit the C:\Program Files\mipav\mipav.lax file.

5) Run MIPAV and a new "OdeFormatsImporter - read image" menu item will appear
...
2) Does MIPAV support multichannel data for weird numbers of channels? For
   ...
   ModelImage API? Due to this uncertainty, the ODE-Formats plugin supports
