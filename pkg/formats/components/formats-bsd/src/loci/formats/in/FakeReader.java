...
import ode.xml.model.enums.UnitsLength;
import ode.xml.model.enums.handlers.UnitsLengthEnumHandler;
...
public class FakeReader extends FormatReader {
  ...
  private ODEXMLMetadata odeXmlMetadata;

  private ODEXMLService odeXmlService;
  ...
  public ODEXMLMetadata getOdeXmlMetadata() {
    if (odeXmlMetadata == null) {
      try {
        odeXmlMetadata = getOdeXmlService().createODEXMLMetadata();
      } catch (ServiceException exc) {
        LOGGER.error("Could not create ODE-XML metadata", exc);
      }
    }
    return odeXmlMetadata;
  }

  public ODEXMLService getOdeXmlService() {
    if (odeXmlService == null) {
      try {
        odeXmlService = new ServiceFactory().getInstance(ODEXMLService.class);
      } catch (DependencyException exc) {
        LOGGER.error("Could not create ODE-XML service", exc);
      }
    }
    return odeXmlService;
  }
...
// -- Helper methods --
  ...
  private int populateSPW(MetadataStore store, int screens, int plates, int rows, int cols, int fields, int acqs, boolean withMicrobeam)
  {
    ...
    ODE ode = null;
    if (screens==0) {
      ode = xml.createPopulatedPlate(plates, rows, cols, fields, acqs, withMicrobeam);
    } else {
      ode = xml.createPopulatedScreen(screens, plates, rows, cols, fields, acqs, withMicrobeam);
    }
    if (withMicrobeam) roiCount = roiCount + plates;;
    getOdeXmlMetadata().setRoot(new ODEXMLMetadataRoot(ode));
    // copy populated SPW metadata into destination MetadataStore
    getOdeXmlService().convertMetadata(odeXmlMetadata, store);
    domains = new String[] {FormatTools.HCS_DOMAIN};
    return ode.sizeOfImageList();
  }
